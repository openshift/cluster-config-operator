...
import (
	...
	// The import below is necessary to ensure that the CCO operator tests are registered with the extension.
	_ "github.com/openshift/cluster-config-operator/test/extended"
)

func main() {
	registry := e.NewRegistry()
	ext := e.NewExtension("openshift", "payload", "cluster-config-operator")
	...
	ext.AddSuite(e.Suite{
		Name:    "openshift/cluster-config-operator/optional/slow",
	...
	ext.AddSuite(e.Suite{
		Name: "openshift/cluster-config-operator/all",
	})
	...
	// "[sig-config-operator] <test name here>",
	)
